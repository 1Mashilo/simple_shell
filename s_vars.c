...
/**
* isChainDelimiter - test if the current char in the buffer is a chain delimiter
...
{
replaceString(&(info->argv[i]), _strdup(s_convert_number(info->status, 10, 0)));
continue;
}

if (!_strcmp(info->argv[i], "$$"))
{
replaceString(&(info->argv[i]), _strdup(s_convert_number(getpid(), 10, 0)));
continue;
}
...
if (node)
{
replaceString(&(info->argv[i]), _strdup(s_strchr(node->str, '=') + 1));
continue;
}
